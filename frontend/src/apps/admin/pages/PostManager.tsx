...
const parseQuery = (searchParams: URLSearchParams): QueryState => ({
  ...
  status: searchParams.get("status") === "draft" || searchParams.get("status") === "published" ? (searchParams.get("status") as QueryState["status"]) : defaultQuery.status,
...
const PostManager = () => {
      ...
      Array.from(
        new Map(
          (data.activities || [])
            ...
            .map((post) => [post.author?.id || "", post.author?.username || "Không rõ tác giả"]),
        ).entries(),
  ...
  const saveDraft = async () => {
    try {
      ...
      const message = getApiErrorMessage(saveError, text.savePostError);
      setError(message);
      toast.error(message);
    } finally {
      setSaving(false);
  ...
  const handlePublish = (post: Post) => mutatePost(() => publishPost(post.id), text.publishPostSuccess, text.publishPostError);

  const handleMoveToDraft = (post: Post) => mutatePost(() => updatePost(post.id, { status: "draft" }), "Đã chuyển bài viết về nháp.", "Không thể chuyển bài viết về nháp.");
  ...
  const handleDelete = async (post: Post) => {
    ...
    await mutatePost(() => deletePost(post.id), text.deletePostSuccess, text.deletePostError);
  };
  ...
  const handleBulkAction = async () => {
    ...
    if (bulkAction === "delete" && !window.confirm(`Xóa ${selectedPosts.length} bài viết đã chọn?`)) {
      return;
    }
    ...
    try {
      ...
      if (failedOperations.length > 0) {
        ...
        const message = `Không thể xử lý ${failedTitles.length} bài: ${failedTitles.join(", ")}`;
        setError(message);
        toast.error(message);
      } else {
        toast.success("Thao tác thành công");
      ...
      const message = getApiErrorMessage(bulkError, "Không thể thực hiện thao tác hàng loạt.");
      setError(message);
      toast.error(message);
    } finally {
      setSelectedIds([]);
    ...
    <div className="space-y-5">
      <div className="rounded-xl border border-slate-200 bg-white p-5">
        <div className="flex flex-col gap-4 border-b border-slate-200 pb-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-2xl font-semibold text-slate-900">Quản lý bài viết</h2>
            <p className="mt-1 text-sm text-slate-500">Tập trung biên tập, kiểm duyệt và quản trị nội dung theo luồng CMS.</p>
          </div>
          <Button onClick={openCreate} className="h-10 rounded-lg bg-red-700 px-4 hover:bg-red-800">
            <Plus className="mr-1.5 h-4 w-4" />+ Tạo bài viết
          </Button>
        </div>
        ...
        {selectedIds.length > 0 ? (
          <div className="mt-4 flex flex-col gap-2 rounded-lg border border-slate-200 bg-slate-50 p-3 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-sm text-slate-700">Đã chọn {selectedIds.length} bài viết</p>
            <div className="flex flex-wrap items-center gap-2">
              <Select value={bulkAction} onValueChange={(value) => setBulkAction(value as typeof bulkAction)}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="draft">Chuyển về nháp</SelectItem>
                      ...
                      <TableRow key={postId} className="align-top transition-colors hover:bg-slate-50/80">
                        ...
                        <TableCell>
                          <Badge variant="outline" className={statusClasses[post.status]}>
                            {post.status === "published" ? statusText.published : statusText.draft}
                          </Badge>
                        </TableCell>
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-44">
                              ...
                              {isAdmin && post.status !== "published" ? (
                                <DropdownMenuItem onClick={() => handlePublish(post)}>{text.actionPublish}</DropdownMenuItem>
                              ) : null}
                              {isAdmin && post.status !== "draft" ? (
                                <DropdownMenuItem onClick={() => handleMoveToDraft(post)}>Chuyển về nháp</DropdownMenuItem>
                              ) : null}
                              {isAdmin ? (
                                <>
                                  <DropdownMenuSeparator />
                ...
                <TableRow>
                  <TableCell colSpan={6} className="py-16">
                    <div className="flex flex-col items-center justify-center text-center">
                      ...
                      <p className="text-base font-medium text-slate-900">Chưa có bài viết nào</p>
                      <p className="mt-1 text-sm text-slate-500">Bắt đầu bằng việc tạo bài viết đầu tiên để xuất bản nội dung.</p>
        ...
        <div className="mt-4 flex flex-col gap-3 border-t border-slate-200 pt-4 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm text-slate-500">
            Hiển thị {(pagination.page - 1) * pagination.limit + (posts.length > 0 ? 1 : 0)}-
            {(pagination.page - 1) * pagination.limit + posts.length} trên {pagination.total} bài viết
          </p>
        ...
        {isFetching && !isLoading ? <p className="mt-2 text-xs text-slate-500">Đang đồng bộ dữ liệu từ máy chủ...</p> : null}
      </div>
