...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        setLoading(true);
        const [cmsData, postsResponse, categoryTree] = await Promise.all([
          getCmsData(),
        ...
        const nextCategoryById = {};
        const walk = (nodes = []) => {
          nodes.forEach((node) => {
            const id = node._id || node.id;
            if (id) {
              nextCategoryById[id] = node.name;
            }
  ...
  const resolvePostCategoryLabel = (post) => {
    const ids = post.categoryIds || [];
    if (!Array.isArray(ids) || ids.length === 0) {
      return "";
    }
    return categoryById[ids[0]] || "";
  };
                ...
                {data.activities.map((item, index) => (
                  ...
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2.5">
                      <h4 className={`${index === 0 ? "text-2xl" : "text-xl"} leading-[1.4] font-bold text-slate-900`}>
                        {item.title}
                      </h4>
                      <span className="text-xs font-medium tracking-wide text-slate-500">{item.date}</span>
                    </div>
                    {resolvePostCategoryLabel(item) ? (
                      <p className="mt-2 text-xs font-medium uppercase tracking-wide text-slate-500">{resolvePostCategoryLabel(item)}</p>
                    ) : null}
